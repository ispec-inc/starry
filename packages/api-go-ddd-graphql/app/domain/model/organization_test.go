...
func TestOrganizationName_Validate(t *testing.T) {
	type (
		give struct {
			name string
		}
	...
	for i := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			name := model.OrganizationName(test.give.name)
			err := name.Validate()
...
func TestOrganizationNameKana_Validate(t *testing.T) {
	type (
		give struct {
			name string
		}
	...
	for i := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			kana := model.OrganizationNameKana(test.give.name)
			err := kana.Validate()
			if test.want.err != nil {
				assert.EqualError(t, test.want.err, err.Error())
			} else {
